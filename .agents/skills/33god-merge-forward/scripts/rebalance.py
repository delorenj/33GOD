...
REPO_ROOT = SKILL_DIR.parents[2]  # <repo>/.agents/skills/<extension>
...
def emit_result(status: str, **details: object) -> None:
    ...
    if os.environ.get("BB_HOOK_HUB") == "off":
        outcome = "succeeded" if status in {"applied", "no-change"} else "failed" if status in {"failed", "rejected"} else "skipped"
        print(json.dumps({"_hook_hub": {"status": outcome, "reason": "merge_forward_" + status.replace("-", "_"), "exit_code": int(outcome == "failed")}, "stdout": ""}))
...
def parse_payload(raw: str, client: str) -> dict:
    ...
    return {
        ...
        "payload": data,
        "received_at": utc_now(),
        "quiet_seconds": 90 if event == "Stop" else 2,
    }
    ...
    if not messages:
        ...
        for role, key in (("USER", "prompt"), ("ASSISTANT", "last_assistant_message")):
            ...
            if isinstance(value, str) and value.strip():
                text = re.sub(r"\s+", " ", value).strip()[:4000]
...
def process_pending(path: Path, dry_run: bool = False) -> int:
    ...
    if processed.exists() and processed.read_text().strip() == fingerprint:
        emit_result("skipped", reason="session evidence already processed", session_id=record["session_id"])
    ...
    with tempfile.TemporaryDirectory(prefix="33god-merge-forward-rebalance-") as temp_dir:
        ...
        # Commit only this worker's allowed files, preserving any unrelated
        # staged changes. A successful mutation cannot deadlock future runs by
        # remaining dirty, which was the original tuner's failure mode.
        paths = [str((SKILL_DIR / item).relative_to(REPO_ROOT)) for item in changed]
        committed = git("commit", "--only", "-m", "chore(merge-forward): apply session tuning", "--", *paths)
        if committed.returncode:
            for relative in changed:
                (SKILL_DIR / relative).write_bytes(baseline[relative])
            emit_result("failed", reason="tuner commit failed", session_id=record["session_id"])
            return 0
        published = git("push", "origin", "HEAD:main")
        if published.returncode:
            emit_result("failed", reason="tuner commit is local; push failed", session_id=record["session_id"])
            return 0
...
def main() -> int:
    ...
    parser.add_argument("--hub", action="store_true", help="supervised foreground execution with structured outcome")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--force", action="store_true", help="skip debounce for a manual dry run")
    args = parser.parse_args()

    if args.hub:
        os.environ["BB_HOOK_HUB"] = "off"
    raw = sys.stdin.read() if args.stdin else args.input.read_text(errors="replace") if args.input.exists() else "{}"
    if not args.dry_run and args.input is not None:
        args.input.unlink(missing_ok=True)
    record = parse_payload(raw, args.client)
    if not within_repo(record["cwd"]):
        emit_result("skipped", reason="session belongs to another repository")
        return 0
    ...
    with lock_path.open("a+") as lock:
        try:
            ...
            emit_result("deferred", reason="another tuner owns the worker")
            return 0
        target = newest_pending() if args.force else wait_until_quiet()
        if target:
            return process_pending(target)
        emit_result("skipped", reason="no pending session")
        return 0
