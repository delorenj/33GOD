...
> **GOD Document** - System-wide developer reference
>
...
- **Agent Heartbeats** — Managed by each agent individually and scheduled through Syringe CLI
...
5. **Holocene Provides Visibility**: Real-time dashboard fetches history from Candystore API and subscribes to live events via WS relay.

6. **Registry as Truth**: `services/registry.yaml` defines service topology

7. **Modular & Autonomous**: Components are small, focused, independently deployable

8. **Agent-Centric**: AI agents are first-class citizens with their own event domains

9. **Observable by Default**: All events flow through Bloodbank for tracing
