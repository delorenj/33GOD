...
| Path | Role |
| --- | --- |
| `~/.agents/hooks/hindsight/hindsight-recall.sh` | UserPromptSubmit — injects recall context, logs `recall` / `recall_skipped` events |
| `~/.agents/hooks/hindsight/hindsight-retain.sh` | PostToolUse (file writes) — retains edit context, logs `retain` events |
| `~/.agents/hooks/hindsight/hindsight-session-end.sh` | Stop — daemonizes the journal writer + legacy session-summary retain |
| `~/.agents/hooks/hindsight/hindsight-journal-write.sh` | Reads telemetry, calls reflect, writes the journal markdown |
| `~/.agents/hooks/lib/hindsight-journal.sh` | Shared `journal_log` + `journal_session_id_from` helpers |
| `~/.agents/hooks/lib/hindsight-bank.sh` | Shared bank resolver (used by all three hooks) |
...
echo '{"session_id":"<uuid>","cwd":"/path/to/cwd"}' \
  | ~/.agents/hooks/hindsight/hindsight-journal-write.sh
```
