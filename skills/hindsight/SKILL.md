...
The Claude Code analog of the OpenClaw plugin. Machine-global hooks under `~/.agents/hooks/hindsight/` auto-recall on every user prompt, auto-retain on every file edit, and emit a per-session journal at `~/.claude/.hindsight-journal/YYYY-mm-dd-h-m-s.md` that explains in plain English what was searched, what bank was resolved, how effective the recall was, and what was retained (with LLM-synthesized rationale for the subjective parts).

The invariant is one Hindsight folder and one script per action. Agent-specific files such as Claude `settings.json` or Codex `config.toml` should point at these canonical scripts; they should not carry forked hook implementations.
...
Retired paths such as `~/.agents/hooks/hindsight.old/` and per-agent hook copies are migration leftovers. Do not add new logic there.
