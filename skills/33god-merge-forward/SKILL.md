...
description: Incremental delivery workflow for the 33GOD monorepo and its component repositories. Use when implementing, integrating, pinning, containerizing, publishing, or composing changes involving Bloodbank, Candystore, Lifecycle, Holocene, Momo, PJangler, Hermes, Toad, CommonProject, Voxxy, or root topology; especially when work risks long-lived branches, broad component freezes, excessive review or security ceremony, submodule or image drift, or delayed merges to main. Produces the smallest observable slice, bounded verification, immediate component and root main merges, and concise status reporting.
---
...
Ship one observable 33GOD improvement at a time and merge it to `main` as soon as its affected behavior is proven.

Optimize for the actual environment:
...
Preserve these boundaries in code, Compose, tests, and docs:

- **Lifecycle** owns deterministic lifecycle truth, transitions, capabilities, obligations, and commands.
- **Momo** chooses and performs legal work. It reports outcomes; it does not determine truth.
- **Holocene** renders system state and offers high-level invocable actions. It is not a lifecycle engine.
- **Bloodbank** owns event schemas, envelopes, and transport. Use real JetStream behavior when testing durability or replay.
- **Candystore** owns append-only event persistence, audit data, and projections.
- **PJangler** owns project identity, registry, bootstrap, and projections.
- **Hermes Template** owns the reusable agent template; fleet runtime truth lives outside project source.
- **The root monorepo** owns topology, exact source/image pins, Compose assembly, cross-component acceptance, and boundary drift checks.

If a requested change redraws one of these boundaries, stop and state that product decision plainly before implementation. Do not let a generic framework silently reassign ownership.
...
Read [references/gates.md](references/gates.md) before selecting acceptance gates or escalating review depth.
...
- root `tasks.md` and the most recently changed relevant artifacts;
...
Keep one root coordination entry in `tasks.md`. Do not create competing ledgers for the same slice.
...
Select the minimum sufficient gates from [references/gates.md](references/gates.md).
...
Run a real transport or persistence test only when the slice claims transport or persistence behavior. For example, durable Bloodbank acceptance must exercise real JetStream, not an in-process substitute.
...
A pull request is optional. Do not require one for this single-user pre-production system.
...
- a change to lifecycle authority or event schema compatibility;
...
Use the project-scoped session-end hook to improve this skill from concrete 33GOD session evidence. The hook must remain asynchronous, debounced, recursion-guarded, and limited to this skill directory.

Apply automatic tuning only from a clean `main` checkout. Build and validate changes in a temporary candidate first, preserve the product authority and pre-production invariants, and never commit or push automatically. Prefer removing or narrowing disproven ceremony over accumulating more rules. Allow per-dev disablement through `.agents/local.json`.

Treat a dirty skill or non-`main` checkout as a deferred update, not a reason to disturb active work.
