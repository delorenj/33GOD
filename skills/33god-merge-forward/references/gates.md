...
Use this reference to choose the smallest gate set that proves the active slice. Do not run every gate in this file.
...
### Bloodbank and Candystore

Use real NATS JetStream when claiming delivery, durability, redelivery, acknowledgement, or offline replay. Prove the event envelope/schema and one persisted or projected receipt. Do not substitute an in-process bus for transport acceptance.

### Lifecycle and Momo

Prove that Lifecycle calculates or validates the transition and emits the legal command. Prove that Momo chooses or executes work and reports the outcome without asserting lifecycle truth.

### Holocene

Prove that Holocene renders source state and invokes a high-level action through the owning service. Reject tests or code that make Holocene calculate lifecycle truth.

### Hermes Template and PJangler

Generate or install one representative project/agent. Prove source, packaged, and installed projections agree. Treat runtime as ignored/external operational state; reject nested runtime gitlinks and destructive replacement of an existing runtime.

### Registry-delivered service

Build and publish a commit-addressed image, record its immutable digest, pull it from a credential-scrubbed environment if first onboarding, start it from root Compose, and verify health. On later slices, digest existence plus affected startup is sufficient.

### Root documentation parity

Search only the changed authority statements and directly affected component docs. A periodic full parity scan belongs at a milestone or after an ownership change, not every implementation commit.
...
- Lifecycle or event-schema authority changed;
- three or more components must change atomically;
- no deterministic acceptance test can prove the behavior.

Record the trigger in `tasks.md`. Absence of a trigger means deterministic validation is the review.
...
- Momo cannot author a Lifecycle transition;
- Holocene cannot become lifecycle truth;
