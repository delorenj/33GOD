#!/usr/bin/env python3
"""Gateway-independent health watchdog for the DeLoNET company reporter."""
...
EXPECTED_JOBS = {
    "ddr:daily",
    "ddr:journal:hermes-fleet-health",
    "ddr:journal:nightly-pr-maintenance",
    "ddr:journal:report-delivery-health",
}
HOME = Path.home()
PROFILE = HOME / ".hermes" / "profiles" / PROFILE_NAME
CONFIG = HOME / ".config" / "delonet-daily-report" / "report.json"
STATE_DIR = HOME / ".local" / "state" / "delonet-daily-report" / "watchdog"
...
def alert_ntfy(codes: list[str]) -> str | None:
    try:
        token = subprocess.run(
            ["op", "read", "op://DeLoSecrets/ntfy/add more/accessToken"],
            check=True,
            capture_output=True,
            text=True,
            timeout=15,
        ...
        body = (
            "Critical DeLoNET company reporter health failure: "
            + ", ".join(sorted(codes))
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "Title": "DeLoNET reporter critical",
                "Priority": "urgent",
                "Tags": "rotating_light",
            },
        )
...
def main() -> int:
    ...
    unexpected = sorted({name for name in names if name and name.startswith("ddr:")} - EXPECTED_JOBS)
    if duplicates:
    ...
    daily = next((job for job in jobs if isinstance(job, dict) and job.get("name") == "ddr:daily"), {})
    ...
    status = {
        ...
        "managed_jobs": sorted(name for name in names if name and name.startswith("ddr:")),
        "delivery": deliver,
