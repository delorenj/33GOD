...
class Provider:
    def __init__(self): self.lane='Todo'; self.revision='revision-1'; self.writes=0; self.lose=False; self.outage=False
    def verify(self, *_):
        if self.outage: raise Rejected('provider unavailable')
    def ticket(self,*_): return {'lane':self.lane,'revision':self.revision}
    def reconcile(self,p,a,t,action,marker): return {'matched': self.lane == action.get('lane'), 'revision':self.revision}
    def apply(self,p,a,t,action,marker):
        self.writes += 1; self.lane=action.get('lane'); self.revision=f'revision-{self.writes+1}'
...
def test_takeover_revision_checked_before_stop(world):
    execute(world)
    with pytest.raises(Rejected): execute(world,operation='takeover',actor='repair',expected_revision=0,payload={'reason':'repair','acceptance':'x','artifact':'y','ticket_revision':world[2].revision})
...
def test_supervised_resume_resets_dispatch_and_question_cannot_be_skipped(world):
    ...
    payload={'question_id':'q','answer':'yes','acceptance':'profile','artifact':'sha2','ticket_revision':world[2].revision}
...
def test_operator_takeover_stops_old_before_new_generation(world):
    execute(world)
    payload={'reason':'repair','acceptance':'profile','artifact':'sha2','ticket_revision':world[2].revision}
...
def test_child_receipts_gate_parent_completion(world):
    c=command(world,payload={'acceptance':{'children':['missing-child-command']},'artifact':'sha1','ticket_revision':world[2].revision})
    world[0].execute(c,'pm')
    for kind in ['spec','quality']:
        execute(world,operation='review',run_id=kind,payload={'artifact':'sha1','acceptance':{'children':['missing-child-command']},'kind':kind,'passed':True,'receipt':kind})
    evidence={'artifact':'sha1','acceptance':{'children':['missing-child-command']},'tests':{'passed':True,'receipt':'test'}}
